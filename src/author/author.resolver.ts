...
import {
  createAuthorArgs,
  CreateAuthorDto,
} from 'src/author/DTO/CreateAuthorDto';
...
export class AuthorResolver {
  ...
  async createAuthor(
    ...
    @Args(createAuthorArgs) createAuthorArgsDto: CreateAuthorDto,
  ) {
    if (user.author) {
      throw new ConflictException(
        'There is already an author associated with the user',
      );
    }
  ...
  async editSelf(
    @CurrentUser() user: User,
    @Args(saveAuhorArgs) saveAuhorArgsDto: SaveAuthorDto,
  ) {
    if (!user.author) {
      throw new NotFoundException(
        'There is no author associated with the user',
      );
    }
    return this.authorService.saveAuthor(user.author.id, saveAuhorArgsDto);
  }
