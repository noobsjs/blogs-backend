...
export class EntryService {
  constructor(
    @InjectModel(ENTRY_MODEL)
    private readonly entrySchema: Model<EntryInterface>,
  ) {}

  async getPaged(limit = 10, offset = 0): Promise<EntryInterface[]> {
    const data = await this.entrySchema
      .find({
        isPublicated: true,
      })
      .skip(offset)
  ...
  async saveEntry(id: string, entry: SaveEntryDto) {
    ...
    if (!oldEntry.publicationDate && entry.isPublicated) {
      oldEntry.publicationDate = new Date().toISOString();
    }
    if (entry.fullBody) {
      oldEntry.fullBody = sanitizeHtml(entry.fullBody, {
